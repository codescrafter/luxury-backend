    ...
    if (response.data.success) {
      ...
      for (const product of products) {
        ...
        if (hasDualLanguageFields) {
          ...
          needsMigrationCount++;
          console.log(
            `   ⚠️  Product ${product._id} (${product.type}) needs migration`,
          );
          console.log(
            `      titleEn: ${!!product.titleEn}, titleAr: ${!!product.titleAr}`,
          );
          console.log(
            `      cityEn: ${!!product.cityEn}, cityAr: ${!!product.cityAr}`,
          );
        }
      }
      ...
      console.log(
        `   📈 Migration progress: ${((migratedCount / products.length) * 100).toFixed(1)}%`,
      );

      if (needsMigrationCount === 0) {
        console.log(
          '🎉 All products have been successfully migrated to dual-language!',
        );
      } else {
        console.log(
          '⚠️  Some products still need migration. Run the migration script.',
        );
      }
    }
  } catch (error) {
...
async function testNewProductCreation() {
  ...
  if (yacht) {
    ...
    console.log(
      `   Description (EN): ${yacht.descriptionEn?.substring(0, 50)}...`,
    );
    console.log(
      `   Description (AR): ${yacht.descriptionAr?.substring(0, 50)}...`,
    );
    ...
    console.log(
      `   Cancellation Policy (EN): ${yacht.cancellationPolicyEn?.join(', ')}`,
    );
    console.log(
      `   Cancellation Policy (AR): ${yacht.cancellationPolicyAr?.join(', ')}`,
    );
    ...
    if (response.data.success && response.data.data.length > 0) {
      ...
      for (const product of products) {
        if (
          ...
          console.log(
            `   ❌ Product ${product._id} has language-specific fields in response`,
          );
          console.log(
            `      titleEn: ${product.titleEn}, titleAr: ${product.titleAr}`,
          );
      ...
      if (!hasLanguageFields) {
        console.log(
          '✅ All products properly cleaned of language-specific fields',
        );
      } else {
        console.log(
          '❌ Some products still contain language-specific fields in response',
        );
      }
    }
  } catch (error) {
    console.error(
      '❌ Language field validation failed:',
      error.response?.data || error.message,
    );
  }
}
...
async function testFallbackBehavior() {
  ...
  try {
    ...
    if (response.data.success && response.data.data.length > 0) {
      ...
      for (const product of products) {
        console.log(
          `   Product: ${product.title} - ${product.city}, ${product.country}`,
        );
        console.log(
  ...
  try {
    ...
    if (updateResponse.data.success) {
      ...
      if (productsResponse.data.success) {
        console.log(
          `✅ Retrieved ${productsResponse.data.data.length} products with Arabic preference`,
        );
      ...
      const switchBackResponse = await axios.post(
        `${BASE_URL}/auth/update-language`,
        {
          language: 'en',
        },
    ...
    console.log(
      '⚠️  Language preference test skipped:',
